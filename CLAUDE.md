...
| `/mt`, `/mt/faces` | MT 스페셜 | 두 팀 점수판(포인트 상시 수동 수정 — 오프라인 게임 반영용) + MT 게임 허브. 태블릿 진행 전제(`.app.wide` 900px). 인물 맞추기: 팀당 15장×2판, 1팀→2팀 교대(sequence [t1,t2,t1,t2]), 3초 카운트다운, 진행자 수동 정답/오답 판정, 맞춘 개수 합계×배율 입력값=포인트 반영. 사진은 `src/assets/mt-faces/{카테고리}/정답이름.jpg` (폴더명=카테고리: 국내/해외/캐릭터 — 스네이크 분배로 각 판에 골고루). 빌드 필요, 60장 미만이면 판 수 자동 축소. 게임2 '이 색 누구게?'(`/mt/simple`): 두 팀 동시 대결, `src/assets/mt-simple/`에 "루피.png"(문제)+"루피2.png"(정답) 짝으로, 정답 공개 후 맞힌 팀 버튼 기록, 맞힌 개수×배율 포인트 반영 |
...
수정/삭제는 **본인 것만** (남의 기도는 불가). 예외: 작성자 프로필이
삭제된 '익명' 기도는 누구나 삭제 가능(정리용). 일정은 누구나 수정/삭제.
...
mt: { createdAt, teams: { t1: {name, points}, t2: {name, points} },
      ...
      simpleGame?: { status, order[이름], pos, revealed, correct{t1,t2} } }
